...
void
Robot::turnRobot(double power , double revolutions) {
	stopDriveMotors();
	power *= -1;
	subsystemDrive->treadLeftFront->Set(power);
	subsystemDrive->treadLeftBack->Set(power);
	...
	while(RobotMap::getDistanceTravelled() < revolutions * FEET_PER_REV) {
...
//	Aim()
	...
	Aim(20);
	moveRobotLinear(.5 , 5); //Goes up to low bar
	moveRobotLinear(.75 , 3.5); //Goes through low bar
	stopDriveMotors(); //Stops so that SAD can got to Auto Aim position

	DriverStation::ReportError("Aim up");

	Aim(70); //Moves to Auto Aim
	moveRobotLinear(.5 , 5); //Goes through other end of low bar
	stopDriveMotors(); //Stops. End autonomous

	DriverStation::ReportError("Turn");

	turnRobot(.5 , 8);
	stopDriveMotors();

	DriverStation::ReportError("End Turn");
