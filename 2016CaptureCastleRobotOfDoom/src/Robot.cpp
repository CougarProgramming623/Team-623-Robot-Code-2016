...
//The Button of the All Mighty Board of Buttons
	...
	if(btnBoard->getSADPos() == SAD_DOWN) {
	...
	if(btnBoard->getPresetPos() != POS_NONE) {
		float newValue = RobotMap::potentiometer->Get();
		switch (btnBoard->getPresetPos()) { // TODO: Find values for potentiometers and replace 0ss
			case POS_PICK_UP:
				newValue = 0;
				break;
			case POS_STORE:
		...
		if(fabs(newValue - RobotMap::potentiometer->Get()) < .05) {
			RobotMap::position->Set(newValue - RobotMap::potentiometer->Get());
		}
	}

	int port_pos = btnBoard->getPortPos();
	if(port_pos != PORTCULIS_NONE) {
		float newValue = RobotMap::potentiometer->Get();
		if(port_pos == PORTCULIS_UP) { // Get Porticulis positions
			newValue = 0;
		}
		else {
			newValue = 0;
...
//	Ball In Button & Ball Out Button
	if(btnBoard->getBallSpinner() == BALL_IN){ // 1
		shoot(-.5);
	}
	else if(btnBoard->getBallSpinner() ==  BALL_OUT){ //-1
		shoot(.25);
	}
